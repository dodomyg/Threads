...
router.post("/login", async (req, resp) => {
  ...
  try {
    ...
    resp
      .status(201)
      .json({ message: "User Logged In", alreadyUser, jwtToken: token });
  } catch (error) {
    resp.status(500).json({ error: "Internal server error" });
  }
});
...
router.get("/jwt", verifyToken, async (req, resp) => {
  ...
  try {
    ...
    const loggedInuser = await USER.findById(userId)
      .populate("followers", "name username dp")
      .populate("following", "name username dp");
...
router.get("/following", verifyToken, async (req, resp) => {
  ...
  try {
    ...
    const getPosts = await POSTS.find({ owner: { $in: following } })
      .sort({ createdAt: -1 })
      .populate("owner", "name username dp")
      .populate("likes", "name username dp")
      .populate({
        path: "comment",
        populate: { path: "userId", select: "name username dp" },
      });

    const myPosts = await POSTS.find({ owner: userId })
      .sort({ createdAt: -1 })
      .populate("owner", "name username dp")
      .populate("likes", "name username dp")
      .populate({
    ...
    const combinedPosts = [...myPosts, ...getPosts];

    resp.status(200).json(combinedPosts);
  } catch (error) {
    console.error(error);
    resp.status(500).json({ error: "Internal Server Error" });
  }
